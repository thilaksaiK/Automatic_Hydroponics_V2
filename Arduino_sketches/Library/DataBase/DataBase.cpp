...
DataBase::storeAt(I2C_eeprom ee, int Address,int Day, int Hour, int Minute, int Temperature, int Humidity, byte LevelArray){
  byte StoreArray[5];
...
DataBase::resetAddress(I2C_eeprom ee){
  	ee.writeByte(0, 0);
  	ee.writeByte(1, 2);
	delay(10);
}
...
int DataBase::getNextAddress(I2C_eeprom ee){
	int ReturnVal = (ee.readByte(0)<<8) | (ee.readByte(1));
	delay(10);
	return ReturnVal;
